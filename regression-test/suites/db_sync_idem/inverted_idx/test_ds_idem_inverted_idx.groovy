...
suite('test_ds_idem_inverted_idx') {
    ...
    helper.addFailpoint('handle_binlog_idempotent:before', 'MODIFY_TABLE_ADD_OR_DROP_INVERTED_INDICES')
    helper.addFailpoint('handle_binlog_idempotent:after', 'MODIFY_TABLE_ADD_OR_DROP_INVERTED_INDICES')

    logger.info('add inverted index')
    sql """
        ALTER TABLE ${tableName}
        ADD INDEX idx_inverted(id) USING INVERTED
    """

    assertTrue(helper.checkShowMapArrayResult("SHOW INDEXES FROM ${tableName}",
        { res -> res.any { it['Key_name'] == 'idx_inverted' && it['Index_type'] == 'INVERTED' } }, 60, "target_sql"))
    ...
    logger.info('build inverted index')

    helper.addFailpoint('handle_binlog_idempotent:before', 'INDEX_CHANGE_JOB')
    helper.addFailpoint('handle_binlog_idempotent:after', 'INDEX_CHANGE_JOB')
    sql """
        BUILD INDEX idx_inverted ON ${tableName}
    """

    sql """
        INSERT INTO ${tableName} VALUES (2, 2, 2)
    """
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${tableName}", 2, 60))

    logger.info('drop inverted index')
