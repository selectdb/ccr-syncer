...
suite("test_ds_idem_modify_comment") {
    ...
    helper.addFailpoint('handle_binlog_idempotent:before', 'MODIFY_COMMENT')
    helper.addFailpoint('handle_binlog_idempotent:after', 'MODIFY_COMMENT')
    
    sql """
        ALTER TABLE ${tableName}
        MODIFY COMMENT "this is a idem modify comment"
        """
    
    assertTrue(checkTableCommentTimesOf(tableName, "this is a idem modify comment", 30))

    sql """
        INSERT INTO ${tableName} VALUES (1, 1)
    """
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${tableName} where id = 1", 1, 60))
}
