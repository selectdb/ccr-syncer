...
suite('test_ds_idem_replace_partition') {
    ...
    helper.addFailpoint('handle_binlog_idempotent:before', 'REPLACE_PARTITIONS')
    helper.addFailpoint('handle_binlog_idempotent:after', 'REPLACE_PARTITIONS')

    sql """
        ALTER TABLE ${tableName}
        ADD TEMPORARY PARTITION `tp3` VALUES [("100"), ("200"))
    """

    assertTrue(helper.checkShowTimesOf("""
                                SHOW TEMPORARY PARTITIONS
                                FROM ${tableName}
                                WHERE PartitionName = "tp3"
                                """,
                                { res -> res.size() > 0 }, 60, "sql"))

    sql """
        ALTER TABLE ${tableName}
        REPLACE PARTITION (`p2`) WITH TEMPORARY PARTITION (`tp3`)
    """
