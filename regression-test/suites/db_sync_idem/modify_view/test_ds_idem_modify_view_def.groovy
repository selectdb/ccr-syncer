...
suite("test_ds_idem_modify_view_def") {
    ...
    def existNewComment = { res -> Boolean
        return res[0][1].contains("COMMENT 'idem_modify_view_comment'")
    }
    ...
    helper.addFailpoint('handle_binlog_idempotent:before', 'MODIFY_VIEW_DEF')
    helper.addFailpoint('handle_binlog_idempotent:after', 'MODIFY_VIEW_DEF')

    sql "ALTER VIEW ${viewName} MODIFY COMMENT \"idem_modify_view_comment\""

    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${viewName}", existNewComment, 30, "target_sql"))

    sql "ALTER VIEW ${viewName} AS SELECT 2"

    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${viewName} where 2 = 2", 1, 60))
}
