...
// under the License.
suite("test_ds_tbl_drop_recover") {
    ...
    def resSql = target_sql "SELECT * FROM ${tableName}_1 WHERE test=0"
    def resSrcSql = sql "SELECT * FROM ${context.dbName}.${tableName}_1 WHERE test=0"
    logger.info("=== {} vs {} ===", resSql.size(), resSrcSql.size())
    assertTrue(resSql.size() == resSrcSql.size())
    ...
    qt_target_sql_content("SELECT * FROM ${tableName}_1")
    qt_sql_source_content("SELECT * FROM ${tableName}_1")
    ...
    order_qt_sql_source_content_2("SELECT * FROM ${tableName}_1")
}
