...
suite("test_ds_view_drop_delete_create") {
    ...
    // delete this job, and recreate it again.
    helper.ccrJobDelete()
    ...
    // A new snapshot must be triggered.
    assertTrue(helper.checkRestoreNumAndFinishedTimesOf("${tableDuplicate0}", num_restore + 1, 60))

    // this create view job must be synced to downstream.
    sql """
        CREATE VIEW view_test_${suffix} (k1, name,  v1)
        AS
        SELECT user_id as k1, name,  SUM(age) FROM ${tableDuplicate0}
