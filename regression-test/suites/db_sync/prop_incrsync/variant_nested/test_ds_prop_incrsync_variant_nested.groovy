...
suite("test_ds_prop_incrsync_incsync_variant_nested") {
    def helper = new GroovyShell(new Binding(['suite': delegate]))
            .evaluate(new File("${context.config.suitePath}/../common", "helper.groovy"))

    logger.info("doris version: 2.1 unsupported: variant nested")
    return
    ...
    assertTrue(res[0][1].contains("\"variant_enable_flatten_nested\" = \"true\""))

    res = sql "desc ${tableNameFull}"

    // target_res = target_sql "desc ${tableNameFull}"

    // assertEquals(res,target_res)

    // target_res = target_sql "SHOW CREATE TABLE ${tableNameFull}"

    // assertTrue(!target_res[0][1].contains("\"variant_enable_flatten_nested\" = \"true\""))
    ...
    res = sql "desc ${tableNameIncrement}"
