...
// under the License.
suite("test_tsa_tbl_res_row_storage") {
    ...
    def res = target_sql "SHOW CREATE TABLE TEST_${context.dbName}.${aliasName}"
    ...
    assertTrue(helper.checkSelectColTimesOf("SELECT * FROM ${aliasName} WHERE test=${test_num}",
                                      3, 30))
    ...
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${aliasName} WHERE test=${test_num}",
                                   1, 30))
    res = target_sql "SELECT cost FROM TEST_${context.dbName}.${aliasName} WHERE test=${test_num}"
