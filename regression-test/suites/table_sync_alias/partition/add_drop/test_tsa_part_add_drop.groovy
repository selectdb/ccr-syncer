...
suite("test_tsa_part_add_drop") {
    ...
    assertTrue(helper.checkShowTimesOf("""
                                SHOW PARTITIONS
                                FROM TEST_${context.dbName}.${aliasTableName}
    ...
    sql "sync"
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${aliasTableName} WHERE test=${test_num}",
                                  insert_num, 30))
    ...
    assertTrue(helper.checkShowTimesOf("""
                                ...
                                """,
                                notExist, 30, "target"))
    def resSql = target_sql "SELECT * FROM ${aliasTableName} WHERE test=3"
