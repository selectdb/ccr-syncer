...
suite('test_tsa_sequence_type') {
    ...
    logger.info("=== Test 1: check sequence type exist with create table sql ===")
    ...
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${context.dbName}.${tableName}",
                                existST, 30, 'sql'))
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE TEST_${context.dbName}.${aliasTableName}",
                                existST, 30, 'target'))
    ...
    sql """
        ...
        values(1, now(), now(), 'Steve', now()), (2, now(), now(), 'Alex', now())
        """

    assertTrue(helper.checkShowTimesOf("SELECT * FROM ${aliasTableName}", { r -> r.size() == 2 }, 60, "target"))

    logger.info('=== Test 3: check data correct ===')

    def res = target_sql "SELECT * FROM ${aliasTableName} ORDER BY user_id"
    ...
    sql """
        insert into ${tableName}  (user_id, date, modify_date, user_name, __DORIS_SEQUENCE_COL__)
        values(1, now(), now(), 'Creeper', now()), (2, now(), now(), 'Pig', now()), (3, now(), now(), 'Dog', now())
        """

    assertTrue(helper.checkShowTimesOf("SELECT * FROM ${aliasTableName}", { r -> r.size() == 3 }, 60, "target"))
    logger.info('=== Test 5: check new data correct ===')

    res = target_sql "SELECT * FROM ${aliasTableName} ORDER BY user_id"
    ...
    sql """
        insert into ${tableName}   (user_id, date, modify_date, user_name, __DORIS_SEQUENCE_COL__)
        values(3, now(), '2025-01-01 12:00:00', 'Wolf', '2025-01-01 12:00:00'), (4, now(), now(), 'Diamond', now())
        """

    assertTrue(helper.checkShowTimesOf("SELECT * FROM ${aliasTableName}", { r -> r.size() == 4 }, 60, "target"))
