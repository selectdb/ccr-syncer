...
// under the License.
suite("test_tsa_auto_incerment_duplicate") {
    ...
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${aliasTableName}", insert_num, 30))

    logger.info("=== Test 3: check auto increment column consistency ===")
    def ures = sql " select * from ${tableName} order by experience "
    def dres = target_sql " select * from ${aliasTableName} order by experience "
    for (int i = 0; i < insert_num; i++) {
