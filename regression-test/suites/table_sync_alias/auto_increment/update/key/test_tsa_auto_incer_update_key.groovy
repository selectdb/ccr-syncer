...
// under the License.
suite("test_tsa_auto_incer_update_key") {
    ...
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${aliasTableName}", existAutoIncrement, 30, "target"))

    logger.info("=== Test 2: insert some data ===")
    sql """
        insert into ${tableName} values (1, 'Alice', 1), (2, 'Bob', 2), (3, 'Carve', 3), (4, 'Colo', 4)
        """
    ...
    sql """
        ...
        """
    logger.info("=== Test 4: update some column ===")
    sql """
        insert into ${tableName} (uuid, user_name) values (4, 'Darvin')
        """
    
    logger.info("=== Test 5: check value update ===")
    helper.checkSelectTimesOf("select * from ${aliasTableName} where user_name = 'Darvin' and uuid = 4", 1, 30)
}
