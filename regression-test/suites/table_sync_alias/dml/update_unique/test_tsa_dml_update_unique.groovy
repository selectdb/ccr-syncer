...
suite("test_tsa_dml_update_unique") {
    ...
    assertTrue(helper.checkShowTimesOf("SELECT * FROM ${aliasName} ", exist, 60, "target"))
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${aliasName}", 3, 30))

    logger.info("=== Test 1: update data ===")
    sql """ update ${tableName} set city = 'd' where user_id = 1; """

    logger.info("=== Test 2: check update data ===")
    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${aliasName} where city = 'd'", 1, 30))
}
