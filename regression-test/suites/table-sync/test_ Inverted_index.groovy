...
suite("test_inverted_index") {
    ...
    sql """
        ...
        (
            ...
            INDEX idx_inverted (`username`) USING INVERTED PROPERTIES("parser" = "english")
        )
        ENGINE=OLAP
    ...
    logger.info("=== Test 1: full update INVERTED index ===")
    httpTest {
        uri "/create_ccr"
    ...
    def checkInverte = { inputRes -> Boolean
        for (List<Object> row : inputRes) {
            if (row[2] == "idx_inverted" && row[10] == "INVERTED") {
                return true
            }
        }
        return false
    ...
    assertTrue(checkShowTimesOf("""
                                ...
                                """, 
                                checkInverte, 30, "target"))
    
    logger.info("=== Test 2: incremental update INVERTED index ===")
    sql """
        ALTER TABLE ${tableName} 
        ADD INDEX idx_only4test(`only4test`) USING NGRAM_BF PROPERTIES("parser" = "english")
        """
    def checkInverte1 = { inputRes -> Boolean
        for (List<Object> row : inputRes) {
            if (row[2] == "idx_only4test" && row[10] == "INVERTED") {
                return true
            }
        }
        return false
