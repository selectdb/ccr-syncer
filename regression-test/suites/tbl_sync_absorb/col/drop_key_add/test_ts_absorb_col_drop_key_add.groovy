...
// under the License.
suite("test_ts_absorb_col_drop_key_add") {
    ...
    sql """
        ALTER TABLE ${tableName}
        ADD COLUMN `value1` INT DEFAULT "1" AFTER `value`
        """
    assertTrue(helper.checkShowTimesOf(""" show columns from ${tableName} """, { r -> return r[3][0] == 'value1' }, 60, "sql"))
    ...
    assertTrue(helper.checkShowTimesOf(""" show columns from ${tableName} """, { r -> return r.size() == 4 }, 60, "target"))
    assertTrue(helper.checkShowTimesOf(""" show columns from ${tableName} """, { r -> return r[0][0] == 'id' }, 60, "target"))
    assertTrue(helper.checkShowTimesOf(""" show columns from ${tableName} """, { r -> return r[3][0] == 'value1' }, 60, "target"))
}
