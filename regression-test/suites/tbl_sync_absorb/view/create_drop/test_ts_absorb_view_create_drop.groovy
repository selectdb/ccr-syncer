...
suite("test_ts_absorb_view_create_drop") {
    ...
    sql """
        CREATE VIEW ${viewName}_1 (k1, name,  v1)
        AS
    ...
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}_1" """, exist, 60, "target"))
    // 0. Insert N data
    sql """
    ...
    assertTrue(helper.checkShowTimesOf(""" select * from ${viewName}_1 """, { r -> r.size() == 10 }, 60, "sql"))


    // 3. Do operation & wait it finishes upstream
    sql """
        CREATE VIEW ${viewName}_2 (k1, name,  v1)
        AS
        SELECT user_id as k1, name, SUM(age) FROM ${tableName}
        GROUP BY k1,name;
        """
    sql "DROP VIEW IF EXISTS ${viewName}_1"
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}_2" """, exist, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}_1" """, notExist, 60, "sql"))
    ...
    assertTrue(helper.checkShowTimesOf(""" select * from ${viewName}_2 """, { r -> r.size() == 15 }, 60, "sql"))
    ...
    assertTrue(helper.checkShowTimesOf(""" select * from ${viewName}_2 """, { r -> r.size() == 15 }, 60, "target"))
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}_2" """, exist, 60, "target"))
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}_1" """, notExist, 60, "target"))
