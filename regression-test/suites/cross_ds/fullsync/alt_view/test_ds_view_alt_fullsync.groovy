...
suite("test_ds_view_alt_fullsync") {
	...
	helper.ccrJobPause()
    sql """
        ALTER VIEW view_test_${suffix}
        (
            k1, name, v1
        )
        AS
        SELECT user_id as k1, name, MAX(age) FROM ${tableDuplicate0}
        GROUP BY k1, name
    """
	...
	logger.info("show force_fullsync ingore alter view dml")
    ...
    def show_view_result = target_sql "SHOW CREATE VIEW view_test_${suffix}"
    logger.info("show view result: ${show_view_result}")
    assertTrue(show_view_result[0][1].contains("MAX("))
