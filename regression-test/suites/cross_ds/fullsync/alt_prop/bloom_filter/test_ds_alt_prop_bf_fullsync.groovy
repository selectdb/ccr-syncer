...
suite("test_ds_alt_prop_bf_fullsync") {
	...
	logger.info("=== Test 4: alter table set property bloom filter columns to test")
    
	helper.ccrJobPause()
	target_sql """
        ALTER TABLE ${tableName} SET ("bloom_filter_columns" = "id");
        """
	def existTarget_BF_FIXED = { res -> Boolean
        return checkShowResult(res, "\"bloom_filter_columns\" = \"id\"")
    }
	
	assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${tableName}", existTarget_BF_FIXED, 60, "target"))
	target_sql "sync"
	...
	sql """
        ALTER TABLE ${tableName} SET ("bloom_filter_columns" = "test");
        """
		
	def existBF_TEST = { res -> Boolean
        return checkShowResult(res, "\"bloom_filter_columns\" = \"test\"")
    }
	
	assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${tableName}", existBF_TEST, 60, "target"))
