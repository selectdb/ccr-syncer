...
suite('test_cds_ps_tbl_drop_create') {
    ...
    """
    sql "INSERT INTO ${tableName} VALUES (5, 500)"

    helper.ccrJobResume()

    assertTrue(helper.checkSelectTimesOf("SELECT * FROM ${tableName}", 1, 60))

    // FIXME: a fullsync is triggered
    // WARN partial sync table tbl_598617586 id not match, force full sync.
    // table id 10840, backup object id 11265 job=test_cds_ps_tbl_drop_create line=ccr/job.go:528
    //
