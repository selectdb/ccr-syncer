...
suite("test_drop_column") {
    def tableName = "tbl_drop_column" + UUID.randomUUID().toString().replace("-", "")
    ...
    assertTrue(checkShowTimesOf("SHOW COLUMNS FROM `${tableName}`", id_column_not_exists, 60, "target_sql"))
    ...
    assertTrue(checkShowTimesOf("SHOW COLUMNS FROM `${tableName}`", value_column_not_exists, 60, "target_sql"))
}
