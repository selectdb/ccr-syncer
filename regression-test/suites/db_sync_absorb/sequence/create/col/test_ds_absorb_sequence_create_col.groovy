...
// under the License.
suite("test_ds_absorb_sequence_create_col") {
    ...
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_1 """, existSC, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_1 """, existSC, 60, "target"))

    // 0. Insert N data
    for (int index = 0; index < insert_num; index++) {
        def str = helper.randomSuffix()
        sql """
            INSERT INTO ${tableName}_1 VALUES (${index}, now(), now(), ${str})
            """
    }
    ...
    for (int index = 0; index < insert_num * 3; index++) {
        def str = helper.randomSuffix()
        sql """
            INSERT INTO ${tableName}_2 VALUES (${index}, now(), now(), ${str})
            """
    }
    ...
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_2 """, existSC, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_2 """, existSC, 60, "target"))
}