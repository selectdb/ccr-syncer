...
// under the License.
suite("test_ds_absorb_sequence_create_type") {
    ...
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_1 """, existST, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_1 """, existST, 60, "target"))

    // 0. Insert N data
    for (int index = 0; index < insert_num; index++) {
        def str = helper.randomSuffix()
        sql """
            INSERT INTO ${tableName}_1  (user_id, date, modify_date, user_name, __DORIS_SEQUENCE_COL__)
    ...
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_2 """, existST, 60, "sql"))
    // 4. Insert N data
    ...
    for (int index = 0; index < insert_num * 3; index++) {
        def str = helper.randomSuffix()
        sql """
            INSERT INTO ${tableName}_2  (user_id, date, modify_date, user_name, __DORIS_SEQUENCE_COL__)
            VALUES (${index}, now(), now(), ${str}, now())
            """
    }
    ...
    assertTrue(helper.checkShowTimesOf(""" SHOW TABLES LIKE "${tableName}_2" """, exist, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName}_2 """, existST, 60, "target"))
}