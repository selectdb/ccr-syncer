...
suite("test_ds_absorb_idx_add_ng_bf") {
    ...
    // Define check for NGRAM_BF existence
    ...
    // 0. Insert initial data
    def values = [];
    for (int index = 0; index < insert_num; index++) {
        values.add("(2, ${index}, 'user_${index}', 'text_data_${index}')")
    }
    sql """
        INSERT INTO ${tableName} VALUES ${values.join(",")}
    """
    assertTrue(helper.checkShowTimesOf(""" select * from ${tableName} """, has_count(insert_num), 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" select * from ${tableName} """, has_count(insert_num), 60, "target"))
    ...
    // 3. Do operation - add NGRAM_BF index
    sql """
    ...
    // Verify NGRAM_BF index was added upstream
    assertTrue(helper.checkShowTimesOf("""
        SHOW CREATE TABLE ${context.dbName}.${tableName}
        """, 
        hasNGBFIndex, 30, "sql"))

    // 4. Insert more data after adding NGRAM_BF index
    sql """
        INSERT INTO ${tableName} VALUES
        (1, 100, "ngbf_user_1", "ngram_searchable_text_1"),
        (1, 101, "ngbf_user_2", "some other searchable content"),
        (1, 102, "ngbf_user_3", "unique phrase")
    """

    // Verify query with substring search works
    assertTrue(helper.checkShowTimesOf(
        """ select * from ${tableName} where value like '%phrase%' """, 
        has_count(1), 30, "sql"))
    ...
    // Check NGRAM_BF index is synced
    assertTrue(helper.checkShowTimesOf("""
        SHOW CREATE TABLE TEST_${context.dbName}.${tableName}
        """,
        hasNGBFIndex, 30, "target"))

    // Verify query works downstream with substring search
    assertTrue(helper.checkShowTimesOf(
        """ select * from ${tableName} where value like '%phrase%' """,
