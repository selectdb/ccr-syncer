...
// under the License.
suite("test_ds_absorb_rollup_rename") {
    ...
    def hasOldRollupAdded = { res -> Boolean
        for (List<Object> row : res) {
            if ((row[0] as String) == "rollup_${tableName}_full_old") {
                return true
            }
        }
        return false
    ...
    def hasNewRollupAdded = { res -> Boolean
        for (List<Object> row : res) {
    ...
    sql """
        ...
        ADD ROLLUP rollup_${tableName}_full_old (id, col2, col4)
        """
    ...
    assertTrue(helper.checkShowTimesOf("DESC ${context.dbName}.${tableName} ALL", hasOldRollupAdded, 30, 'sql'))
    assertTrue(helper.checkShowTimesOf("DESC TEST_${context.dbName}.${tableName} ALL", hasOldRollupAdded, 30, 'target'))
    ...
    sql """
        ALTER TABLE ${tableName} RENAME ROLLUP rollup_${tableName}_full_old rollup_${tableName}_full_new
        """

    assertTrue(helper.checkShowTimesOf("DESC ${context.dbName}.${tableName} ALL", hasNewRollupAdded, 30, 'sql'))
    // 4. Insert N data
    ...
    assertTrue(helper.checkShowTimesOf("DESC TEST_${context.dbName}.${tableName} ALL", hasNewRollupAdded, 30, 'target'))
}