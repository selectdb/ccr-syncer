...
suite("test_dsd_view_alter") {
    ...
    sql "ALTER VIEW ${viewName} AS SELECT user_id as k1, name FROM ${tableName} GROUP BY k1,name;"
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}" """, exist, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" select * from ${viewName} """, { r -> r.size() == 5 }, 60, "sql"))
    ...
    assertTrue(helper.checkShowTimesOf(""" SHOW VIEWS LIKE "${viewName}" """, exist, 60, "target"))
    assertTrue(helper.checkShowTimesOf(""" select * from ${viewName} """, { r -> r.size() == 10 }, 60, "target"))
