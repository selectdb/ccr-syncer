...
// under the License.
suite("test_tsa_absorb_tbl_replace_false") {
    ...
    helper.ccrJobDelete("${tableName}_1")
    helper.ccrJobCreate("${tableName}_1")
    ...
    // 1. Pause ccr job
    helper.ccrJobPause("${tableName}_1")
    ...
    sql """
        alter table ${tableName}_1 replace with table ${tableName}_2 PROPERTIES('swap' = 'false');
        """
    ...
    res = target_sql " select * from ${aliasTableName}";
    for (int index = 0; index < insert_num * 3; index++) {
        assertTrue(checkValue(res, index, index + insert_num * 3))
    }
}