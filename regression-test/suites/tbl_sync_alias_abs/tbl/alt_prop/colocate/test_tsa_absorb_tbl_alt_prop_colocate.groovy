...
// under the License.
suite("test_tsa_absorb_tbl_alt_prop_colocate") {
    ...
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${tableName}", notExistGrooup1, 60, "sql"))
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${aliasTableName}", notExistGrooup1, 60, "target"))

    sql """
        ALTER TABLE ${tableName} SET ("colocate_with" = "test_group_1")
        """
    ...
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${tableName}", existGrooup1, 60, "sql"))
    // don't sync
    assertTrue(helper.checkShowTimesOf("SHOW CREATE TABLE ${aliasTableName}", notExistGrooup1, 60, "target"))
