...
suite("test_tsa_absorb_mv_drop") {
    ...
    // Verify MV is created in target
    ...
    // 3. Do operation & wait it finishes upstream - drop the materialized view
    sql """
        DROP MATERIALIZED VIEW mv_${tableName} ON ${tableName}
        """
    
    // Verify the MV is dropped on source
    assertTrue(helper.checkShowTimesOf("""
                                SHOW CREATE MATERIALIZED VIEW mv_${tableName}
                                ON ${tableName}
                                """,
                                { res -> res.size() == 0 }, 30, "sql"))
    ...
    // Verify MV is dropped in target
    assertTrue(helper.checkShowTimesOf("""
                                SHOW CREATE MATERIALIZED VIEW mv_${tableName}
                                ON ${aliasTableName}
                                """,
