...
// under the License.
suite("test_tsa_absorb_part_add") {
    ...
    def existNewPartition = { res -> Boolean
        return checkShowResult(res, """PARTITION p100 VALUES [("10"), ("100")))""")
    }
    def notExistNewPartition = { res -> Boolean
        return !checkShowResult(res, """PARTITION p100 VALUES [("10"), ("100")))""")
    }
    sql """
    ...
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName} """, existNewPartition, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" show create table ${aliasTableName} """, notExistNewPartition, 60, "target"))
    ...
    assertTrue(helper.checkShowTimesOf(""" show create table ${tableName} """, existNewPartition, 60, "sql"))
    assertTrue(helper.checkShowTimesOf(""" show create table ${aliasTableName} """, existNewPartition, 60, "target"))
}