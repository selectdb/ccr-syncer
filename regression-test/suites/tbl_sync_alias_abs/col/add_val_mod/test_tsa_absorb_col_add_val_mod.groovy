...
// under the License.
suite("test_tsa_absorb_col_add_val_mod") {
    ...
    assertTrue(helper.checkShowTimesOf(""" show columns from ${aliasTableName} """, { r -> return r[3][0] == 'value1' }, 60, "target"))
    assertTrue(helper.checkShowTimesOf("SHOW COLUMNS FROM `${aliasTableName}`", id_is_big_int, 60, "target"))
    assertTrue(helper.checkShowTimesOf("SHOW COLUMNS FROM ${aliasTableName}", value_is_big_int, 60, "target"))
}
