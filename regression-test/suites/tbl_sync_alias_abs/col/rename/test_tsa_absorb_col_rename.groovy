...
// under the License.
suite("test_tsa_absorb_col_rename") {
    ...
    def insert_num = 5
    def newColName = 'new_col'
    def oldColName = 'old_col'
    ...
    sql """
        ...
        RENAME COLUMN ${oldColName} ${newColName}
        """
    ...
    assertTrue(helper.checkShowTimesOf("SHOW COLUMNS FROM ${tableName}", has_column(newColName), 60, "sql"))
    assertTrue(helper.checkShowTimesOf("SHOW COLUMNS FROM ${tableName}", not_has_column(oldColName), 60, "sql"))
    assertTrue(helper.checkShowTimesOf("SHOW COLUMNS FROM ${aliasTableName}", has_column(newColName), 60, "target"))
    assertTrue(helper.checkShowTimesOf("SHOW COLUMNS FROM ${aliasTableName}", not_has_column(oldColName), 60, "target"))
}
