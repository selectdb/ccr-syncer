...
// under the License.
suite("test_ts_col_default_value") {
    def helper = new GroovyShell(new Binding(['suite': delegate]))
            .evaluate(new File("${context.config.suitePath}/../common", "helper.groovy"))

    def tableNameAgg = "test_ts_col_default_value_tbl_agg"
    def tableNameDup = "test_ts_col_default_value_tbl_dup"
    ...
    helper.ccrJobDelete(tableNameAgg)
    helper.ccrJobCreate(tableNameAgg)
    ...
    helper.ccrJobDelete(tableNameDup)
    helper.ccrJobCreate(tableNameDup)

    assertTrue(helper.checkRestoreFinishTimesOf("${tableNameDup}", 30))
