...
func (j *IngestBinlogJob) handleTablet(binlogVersion int64, destPartitionId int64, srcTablet *TabletMeta, destTablet *TabletMeta) {
	...
	for _, tableRecord := range j.tableRecords {
		...
		for _, partitionRecord := range tableRecord.PartitionRecords {
			...
			for tabletIndex, destTablet := range destTablets {
				...
				j.handleTablet(binlogVersion, destPartitionId, srcTablet, destTablet)
				if j.Error() != nil {
					break
				}
			}
